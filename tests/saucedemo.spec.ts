...
test ("Acessar uma página e procurar por um link(checkbox)", async ({ page }) => {
    ...
    const checkBoxLink = page.locator('a[href="/checkboxes"]');
...
test ("Ações básicas (hovers)", async ({ page }) => {
    ...
    const figure1 = page.locator('div.figure').nth(0);
    const figure2 = page.locator('div.figure').nth(1);	
    const figure3 = page.locator('div.figure').nth(2);
    ...
    await expect(hoverFigure2).toBeHidden();
    await expect(hoverFigure3).toBeHidden();

    await figure2.hover();
    await expect(hoverFigure1).toBeHidden();
